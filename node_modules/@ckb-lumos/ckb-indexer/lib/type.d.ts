...
import { Cell, Hexadecimal, HexString, QueryOptions, OutPoint, HexNumber, Output, TransactionWithStatus, Script, Tip } from "@ckb-lumos/base";
...
export interface GetCellsSearchKey<WithData extends boolean = boolean> extends SearchKey {
    withData?: WithData;
}
export interface GetTransactionsSearchKey<Group extends boolean = boolean> extends SearchKey {
...
export interface IndexerCellWithoutData extends Omit<IndexerCell, "outputData"> {
    outputData: null;
}
...
export declare type GetCellWithTerminator = (searchKey: SearchKey, terminator?: Terminator, searchKeyFilter?: SearchKeyFilter) => Promise<GetCellsResults>;
...
export declare type GetCellsRpc = <WithData extends boolean = true>(searchKey: GetCellsSearchKey<WithData>, order: Order, limit: HexString, cursor?: string) => Promise<GetLiveCellsResult<WithData>>;
export declare type GetTransactionsRpc = <Grouped extends boolean = false>(searchKey: GetTransactionsSearchKey<Grouped>, order: Order, limit: HexString, cursor?: string) => Promise<IndexerTransactionList<Grouped>>;
